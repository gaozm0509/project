...
+ (CGRect)frameWithOutNavTab{
    ...
    frame.size.height -= (44+44+20);//减去状态栏、导航栏、Tab栏的高度
    return frame;
}
+ (CGRect)frameWithOutNav{
    CGRect frame = kScreen_Bounds;
    frame.size.height -= (44);//减去状态栏、导航栏的高度
    return frame;
}
...
//        加载失败
        if (!_reloadButton) {
            _reloadButton = [[UIButton alloc] initWithFrame:CGRectMake((maxWidth -160)/2, CGRectGetMaxY(_tipLabel.frame), 160, 40)];
            _reloadButton.layer.borderColor = [UIColor whiteColor].CGColor;
            _reloadButton.layer.cornerRadius = 8;
            _reloadButton.layer.borderWidth = 1;
            [_reloadButton setTitle:@"再试一下" forState:UIControlStateNormal];
        ...
        [_monkeyView setImage:[UIImage imageNamed:@"icon_none01"]];
        _tipLabel.text = @"网络出问题了哦";
    }else{
//        空白数据
        if (_reloadButton) {
        ...
        switch (blankPageType) {
            ...
            {
                imageName = @"icon_none02";
                tipStr = @"这里空空的~";
            }
                break;
            case EaseBlankPageTypeRecord:
            {
                imageName = @"icon_none03";
                tipStr = @"还没有记录哦~";
            }
                break;
            case EaseBlankPageTypeList:
            {
                imageName = @"icon_none03";
                tipStr = @"还没有产品~";
            }
